...
const fakeDom = require("jsdom");
require("google-closure-library");

var jsdom = new fakeDom.JSDOM(``, {
  url: "http://localhost"
});
for (const property of Object.getOwnPropertyNames(jsdom.window)) {
 ...
 */
 function assertSanitizedHtml(originalHtml, expectedHtml, opt_sanitizer) {
  const sanitizer = opt_sanitizer || new goog.html.sanitizer.HtmlSanitizer.Builder().build();
  const sanitized = goog.html.SafeHtml.unwrap(sanitizer.sanitize(originalHtml));
  if (sanitized != expectedHtml) {
    console.info(`${testsPerformed} tests passed but...`);
    throw new Error(`Not equal: ${expectedHtml} != ${sanitized}`);
  }
  testsPerformed++;
}

/** Allowed content is let through */
...
console.info(`${testsPerformed} tests passing`);